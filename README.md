...
**Current state: milestone 7 — the round.** It's a game now. When a Detective and a Spy are both in, a **10-minute round** starts. The Detective hunts the spy (interviews, cameras, the magnifier) and accuses via the Guess panel; **two guesses**, wrong twice and the spy wins. A **correct accusation** wins for the Detective; if the **clock runs out** the Detective also wins (the spy failed to finish). The round ends with a reveal screen. Still to come: the Spy's **tasks** (their proactive win path) and the rest of the ability kit.
